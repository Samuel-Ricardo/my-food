...
internal class CourierOrder {
    ...
    fun assignToCourier(command: AssignCourierOrderToCourierCommand) {
        if (CourierOrderState.CREATED == state) {
            ...
            throw UnsupportedOperationException("Courier order can't be assigned to courier: STATE is not CREATED");
        }
    }
    ...
    @CommandHandler
    fun markOrderAssigned(command: MarkCourierOrderAsAssignedInternalCommand) {
        if (CourierOrderState.ASSIGN_PENDING  == state) {
            ...
            throw UnsupportedOperationException("Courier order can't be assigned to courier: STATE is not ASSIGN_PENDING");
        }
    }
